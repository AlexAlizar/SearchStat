...
public class PersonPageRankDAO {
    ...
    //    return (Integer) session.save(new PersonPageRank(person, page, rank));
        SQLQuery query = session.createSQLQuery("INSERT into person_page_rank(person_id, page_id, rank) VALUES ("+person.getId()+" , "+page.getId()+", "+rank+")");
        return query.executeUpdate();

    }

    public PersonPageRank getPersonPageRank(Person person, Page page) {
        SQLQuery query = session.createSQLQuery("SELECT * FROM person_page_rank where person_id="+person.getId()+" AND page_id="+page.getId());
    ...
    public void updateRank(Person person, Page page, int rank) {
        SQLQuery query = session.createSQLQuery("UPDATE person_page_rank set rank="+rank+" where person_id="+person.getId()+" AND page_id="+page.getId());
        query.executeUpdate();
    }
}
